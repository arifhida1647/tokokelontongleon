...
class Stokout extends Controller
{
    protected $modelStok;
    ...
    public function simpan()
    {
        ...
        if ($validasi->withRequest($this->request)->run()) {
            ...
            if ($item && $item['stok'] < $jumlah) {
                $hasil['sukses'] = false;
                $hasil['error'] = 'Stok tidak mencukupi untuk jumlah yang dimasukkan max : '.$item['stok'];
    ...
    public function index()
    {
        ...
        $data['dataItem'] = $this->modelItems->orderBy('id', 'desc')
        ->where('stok >',"0")
        ->findAll();
        return view('stokout_view', $data);
    }
}
